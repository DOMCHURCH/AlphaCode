...
def test_sma_slope_is_nan_on_insufficient_history():
    """A 200-day SMA with only a handful of days of existence has a noise slope.
    slope() must return NaN there (leading-NaN window), so the gate rejects a
    name it cannot honestly evaluate rather than passing it on a degraded trend.
    This is the intended collapse-to-zero on short history -- the pipeline blocks
    such a run upstream (see test_pipeline_e2e)."""
    n = 207  # < 200 + 21, so the SMA200 slope window straddles the warmup
    dates = pd.bdate_range(end="2026-07-31", periods=n)
    rng = np.random.default_rng(7)
    tickers = [f"T{i:03d}" for i in range(50)]
    close = pd.DataFrame(
        {t: 100 * np.cumprod(1 + rng.normal(0.001, 0.012, n)) for t in tickers},
        index=dates,
    )
    sl = ind.slope(ind.sma(close, 200), 21)
    assert sl.isna().all(), "slope must be NaN when the SMA200 has barely warmed up"
